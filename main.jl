...
if scenario == "backwards"
	...
	lObPlot = [   [ [-20;5], [-spot_w;5], [-spot_w;-5], [-20;-5], [-20;5] ]  ,
	 	  [ [spot_w;5], [20;5], [20;-5], [spot_w;-5], [spot_w;5] ] ,
		  [ [-20;15], [20;15], [20;road_w], [-20;road_w], [-20;15] ]		]		#vetices given in CLOCK-WISE direction

    # for optimization problem
	lOb = [   [ [-20;5], [-spot_w;5], [-spot_w;-5]]  , 
	 	  [ [spot_w;-5] , [spot_w;5] , [20;5] ] , 
		  [ [20;road_w], [-20;road_w]]	]		#vetices given in CLOCK-WISE direction
...
#calculate compuation time
Timelist = Float64[]
astarTimelist = Float64[]
obcaTimelist = Float64[]
cal_pathlength = Float64[]
# for p in 1:10
# call Hybrid A*
tic()
...
rx_sampled = rx[1:sampleN:end] # sampleN=3
# 
...
if exitflag10==1
	...
	#plotTraj(rx',ry',length(rx)-1,ego,L,nObPlot,vObPlot,lObPlot,"Trajectory generated by Hybrid A-star",1)
end
...
push!(Timelist, Float64(totTime))
push!(astarTimelist,Float64(timeHybAstar))
push!(obcaTimelist,Float64(time10))

path_length = 0.0
cal_size = collect(size(xp10'))[1] -1
for z in 1:cal_size
	#calculate length of the path
	path_length += sqrt((xp10'[z,1]-xp10'[z+1,1])^2+(xp10'[z,2]-xp10'[z+1,2])^2)
	#println(path_length)
end
push!(cal_pathlength,Float64(path_length))

# end
println("total timelist : ",Timelist)
println("astar timelist : ",astarTimelist)
println("obca timelist : ",obcaTimelist)
println("path_length : ",cal_pathlength)
